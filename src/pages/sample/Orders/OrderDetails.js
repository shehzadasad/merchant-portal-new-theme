...
import InstallmentsDetailsTable from './InstallmentsDetailsTable'

const columns = [
  { id: 'id', label: 'ID', minWidth: 50, align: 'left' },
  { id: 'name', label: 'Product', minWidth: 250, align: 'center' },
  {
    id: 'quantity',
    label: 'Quantity',
    minWidth: 50,
...
const InstallmentsColumns = [
  {
    id: 'installement_number',
    label: 'Installement Number',
    minWidth: 70,
    align: 'left',
  },
  {
    id: 'installement_amount',
    label: 'Installement Amount',
    minWidth: 80,
    align: 'left',
  },
  {
    id: 'installement_date',
...
const OrderDetails = () => {
  document.title = 'Order Details | QisstPay - Merchants'
  const { id } = useParams()
  ...
  const fetchOrderDetailsHelper = (id) => {
    dispatch(fetchOrderDetails(id, userDetail.id))
  }
  ...
  const OrderDetailsTableMultiVendors = ({ data }) => {
  ...
  return (
    <Container>
      ...
      {orderDetails && orderDetails.order_items_by_merchant ? (
        <OrderDetailsTableMultiVendors
          data={orderDetails.order_items_by_merchant}
        />
      ) : (
        <OrderDetailsTable
      ...
      >
        ...
        {orderDetail && orderDetail.payment_method === 'EasyPaisa' ? (
          <Grid
            container
            sx={{
              padding: 5,
          ...
          >
            ...
            <Grid item xs={8} md={8} lg={8}>
              <Typography variant='p' component='p' fontSize={14}>
                {orderDetail && orderDetail.ep_account_no}
              </Typography>
            </Grid>
          </Grid>
        ) : (
      ...
      {orderDetails.merchant_installment_plans && (
        <>
          <Typography variant='h2' component='h3' marginTop={8}>
            Installements
          </Typography>
          <InstallmentsDetailsTable
