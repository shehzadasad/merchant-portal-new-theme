...
const EditModal = ({
  ...
  const [DropDownValue, setDropDownValue] = useState({
    value: data?.credentials?.type ? data.credentials.type : 'Choose Type',
    label: data?.credentials?.type ? data.credentials.type : 'Choose Type',
  })
  const [credentials, setCredentials] = useState({
    [data.required_credentials?.options.default ??
    Object.keys(data.required_credentials?.options)[0]]:
      data.credentials[data.required_credentials?.options.default],
  })

  useEffect(() => {
    if (data.credentials.type) {
      handleCredentials(
        data.credentials.type,
        data.required_credentials?.options[data.credentials.type],
        data.credentials[
          data.required_credentials?.options[data.credentials.type]
        ]
      )
  ...
  const handleCredentials = (type, key, data = false) => {
    setCredentials({
      type: type,
      [key]: data !== false ? data : '',
    })
  ...
  const UpdateThirdPartyItem = () => {
    toast.info('Updating...')
    var myHeaders = new Headers()
    myHeaders.append('Content-Type', 'application/json')

    var raw = JSON.stringify({
      merchant_app_id: data.id,
    ...
    fetch(
      process.env.REACT_APP_API_URL + 'merchant/third-party/update',
      requestOptions
    )
      ...
      >
        ...
        >
          <Typography
            ...
            fontSize={20}
            fontWeight={'bold'}
            onClick={() => console.log(data, credentials)}
          >
            Connect Integration
          </Typography>
        ...
        {!data?.required_credentials.type && (
          <Box mt={'15px'}>
            <TextField
              ...
              placeholder='key'
              defaultValue={
                credentials[data?.required_credentials?.options.default]
              }
              onChange={(e) => {
                let array = credentials
